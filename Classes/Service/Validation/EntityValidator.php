...
class EntityValidator
{
    ...
    public function validate($request)
    {
		...
		{
            throw new ValidationException('Given object type with className '.$className.' could not be validated.');
		}
    }

    private function validateAcquirerStatusRequest(AcquirerStatusRequest $input)
    {
		$length = strlen($input->getTransactionID());
        if($length !== 16)
            throw new ValidationException("Transaction.transactionID length=".$length." instead of 16 .");
    ...
    private function validateAcquirerTransactionRequest(AcquirerTransactionRequest $input)
    {
        ...
        if ($length !== 1 || $matches[0] !== $input->getIssuerID())
            throw new ValidationException("Issuer.issuerID (".$input->getIssuerID().") does not match format.");
    ...
    private function validateTransaction(Transaction $transaction){
        ...
        if ($length !== 1 || $matches[0] !== $transaction->getPurchaseId())
            throw new ValidationException("Transaction.purchaseID (".$transaction->getPurchaseID().") does not match format.");
